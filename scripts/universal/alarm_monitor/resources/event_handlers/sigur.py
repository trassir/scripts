...
class SigurEventsHandler:
    def __init__(self, obj_storage, sigur_alarm_nodes, callback=None):
        self.obj_storage = obj_storage
        self.sigur_alarm_nodes = sigur_alarm_nodes
    ...
    def event_handler(self, event):
        ...
        try:
            ...
            logger.debug("Can't get name of sigur node, node origin (%s) --> node name. Err: %s", node_name, err)

        if self.sigur_alarm_nodes:
            if node_name not in self.sigur_alarm_nodes:
                logger.debug(
                    "node name (%s) not in alarm nodes list: %s",
                    node_name,
                    self.sigur_alarm_nodes,
                )
                return
        ...
        if not channel_full:
            logger.debug(
                "Can't get associated channel for node name: %s, break.", node_name
                )
            return

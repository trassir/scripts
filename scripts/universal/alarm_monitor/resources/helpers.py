...
def __check_script_error(logger):
    if stats["last_error"]:
        logger.error(
            "Script disabled with error on line %s: %s",
            stats["last_error_line"],
            stats["last_error"],
        )
...
def init_logger(name, debug=False, duplicate_filter=False):
    ...
    logger.info(
        "Starting script %s (%s), helpers v%s", script_.name, script_.guid, __version__
    )
...
def set_script_name(name=None):
    ...
    if script.name in DEFAULT_SCRIPT_NAMES:

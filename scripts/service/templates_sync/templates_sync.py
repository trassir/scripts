# -*- coding: utf-8 -*-
# <h3>Templates synchronization</h3><br>
#
# <code>
# Version: 0.3.1<br>
# Author: A.A.Trubilin, DSSL<br>
# E-mail: a.trubilin@dssl.ru<br>
# Help: <a href="https://scripts.page.link/template_sync">
# https://scripts.page.link/template_sync</a>
# </code>
"""
<parameters>
    <company>AATrubilin</company>
    <title>template_sync</title>
    <version>0.3.1</version>

    <parameter>
        <id>FROM_SERVER</id>
        <type>server</type>
        <name>Server with templates</name>
    </parameter>
    <parameter>
        <id>SYNC_TIME</id>
        <type>integer</type>
        <name>Sync time, sec</name>
        <value>60</value>
        <min>10</min>
        <max>99999</max>
    </parameter>
    <parameter>
        <id>TEMPLATES</id>
        <type>objects</type>
        <name>Only selected templates (optional)</name>
    </parameter>
...
if not FROM_SERVER:
    raise ParameterError("Server not selected")

try:
    settings("/{}".format(FROM_SERVER))
except KeyError:
    raise ParameterError("Server {} not found".format(FROM_SERVER))


class TemplateSync(py_object):
    """Templates synchronization

    Synchronize templates from remote server
    You can sync all templates or only selected

    Args:
        from_server (str) : Remote server guid
        sync_time (int, optional) : Delay between synchronize, sec
            default: sync_time = 60
        templates (str) : Comma spaced template names from parameters
            default: templates=""
        host_api (module, optional) : Trassir host module, default: host_api=host

    Attributes:
        local_templates (list) : List of synchronized templates name
    """
    def __init__(self, from_server, sync_time=60, templates="", host_api=host):
        self.local_templates = []
        self._working = False

        self._templates = None
        if templates:
            self._templates = templates.split(",")

        self._host_api = host_api
        self._from_server = from_server
        self._sync_time = sync_time
        self._server_name = self._host_api.settings("/{}".format(from_server)).name

    def get_server_templates(self):
        """Returns all templates from server"""
        templates = {}
        try:
            settings_templates = self._host_api.settings("/{}/templates".format(self._from_server))
        except KeyError:
            self._host_api.error("TemplateSync: Server {} is unavailable".format(self._server_name))
            settings_templates = None

        if settings_templates is not None:
            for obj in settings_templates.ls():
                if self._templates is None or obj.name in self._templates:
                    try:
                        templates[obj.name] = settings_templates.cd(obj.guid)["content"]
                    except KeyError:
                        continue
    ...
    def get_local_templates(self):
        """Returns local tempaltes"""
        templates = {}

        for obj in self._host_api.settings("templates").ls():
            if self._templates is None or obj.name in self._templates:
                templates[obj.name] = GUITemplate(obj.name)

        return templates

    def update_local_templates(self):
        ...
        for name, content in templates.iteritems():
            local_tmpl = local_templates.pop(name, None)
            if local_tmpl is None:
                local_tmpl = GUITemplate(name)
            local_tmpl.content = content

        for _, tmpl in local_templates.iteritems():
            tmpl.delete()

        self.local_templates = templates.keys()

    def _worker(self, timer=0):
        """Update templates loop"""
        if not timer:
            timer = self._sync_time
            self.update_local_templates()

        self._host_api.stats()["run_count"] = timer
        timer -= 1

        if self._working:
            self._host_api.timeout(1000, lambda: self._worker(timer))
...
tmpl_sync = TemplateSync(FROM_SERVER, SYNC_TIME, TEMPLATES)
tmpl_sync.start()

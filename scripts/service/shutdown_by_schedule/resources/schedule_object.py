...
class ScheduleObject(object):
    ...
    def __init__(self, name_or_guid, color_change_handler=None, on_ready_handler=None, tries=5):
        """

        Args:
            name_or_guid (str): Имя расписания или его guid
            ...
            on_ready_handler (callable, optional): Хэндлер успешной загрузки расписания, по умолчанию "None".
            tries (int, optional): Кол-во попыток загрузить расписание, таймаут между
                попытками 1000мс. По умолчанию `5`
        ...
        self._name = name_or_guid
        self._load_object(tries=tries)
    ...
    def _load_object(self, tries=5):
        ...
        for sett in host.settings("scripts").ls():
            if sett.type == "Schedule" and sett["enable"] and (sett.name == self._name or sett.guid == self._name):
        ...
        try:
            ...
            if callable(self.on_ready_handler):
                self.on_ready_handler(self)

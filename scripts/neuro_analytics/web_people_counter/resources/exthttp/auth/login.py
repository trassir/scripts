...
class LoginHandler(BaseHandler):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.debug("request.GET (%s) = %s", type(request.GET).__name__, request.GET)
        ...
        if not username:
            ...
            if SERVER_GUID == "client":
                ...
                if response.status_code == 200:
                    ...
                    if data["success"]:
                        backref = request.GET.pop("backref", "index")
                        backref += "?sid={sid}".format(sid=data["sid"])
                        request.GET.pop("sid", None)
                        for key, value in request.GET.iteritems():
                            backref += "&%s=%s" % (key, value)
                        return HttpResponseRedirect("/s/{basename}/{backref}".format(

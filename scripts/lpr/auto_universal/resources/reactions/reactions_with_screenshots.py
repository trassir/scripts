        ...
        for img_source, imgs_list in imgs.iteritems():
            for _image in imgs_list:
                if not _image:
                    continue
                path = bytes_to_file(
                    _image, base_folder, prefix="{}_".format(img_source)
                )
                if path:
...
class ReactionsWithScreenshots(Executor):
    ...
    def __init__(
        self, delta, base_folder, online_shot=False, save_screen=False, figures=False
    ):
    ...
    def send_shots(self, task_guid, state):
        ...
        try:
            ...
            if (
                images_paths
                and task.get("images")
                and isinstance(task.get("images"), dict)
            ):
            ...
            for s_name, sender in self.senders.iteritems():
                sender.send(
                    state, images_paths, alarm_messages, channel=task.get("channel")
                )
    ...
    def execute(self, channel, ts, alarm_messages, *args, **kwargs):
        ...
        channel_name = re.sub(
            self.spec_symbols, "_",  channel.name
        )
